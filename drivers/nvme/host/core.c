...
static bool nvme_multi_css_en(struct nvme_ctrl *ctrl)
{
	return (ctrl->ctrl_config & NVME_CC_CSS_MASK) == NVME_CC_CSS_CSI;
}

static bool nvme_multi_css_supp(struct nvme_ctrl *ctrl)
{
	return NVME_CAP_CSS(ctrl->cap) & NVME_CAP_CSS_CSI;
}
		...
		struct nvme_ns_ids *ids)
{
	...
	if (status) {
		dev_warn(ctrl->device,
		  ...
		  * When having support for multi css, then an error is fatal,
		  * since the command is needed in order to get the CSI.
		  */
		if (!nvme_multi_css_supp(ctrl) && status > 0)
			status = 0;
		goto free_data;
	}
...
free_data:
	if (nvme_multi_css_supp(ctrl) && !csi_seen) {
		dev_err(ctrl->device, "Command set not reported for nsid:%d\n",
			nsid);
		status = -EINVAL;
	} else {
		dev_err(ctrl->device, "NSID: %d reports command set: %d\n",
			nsid, ids->csi);
	}
	kfree(data);
	return status;
		...
		struct nvme_id_ns *id, struct nvme_ns_ids *ids)
{
	...
	if (nvme_multi_css_supp(ctrl) || ctrl->vs >= NVME_VS(1, 3, 0))
